(function () {
  ...
  const cardsRoot = document.getElementById("ativos-cards");
  ...
  const TERMINAL_STATUS_PREFIXES = [
    "conclu",
    "desist",
    "deslig",
    "transfer",
  ...
  function prepareRows(rawRows) {
    return rawRows
      .map((row) => {
        ...
        const admissionYear = extractYear(row.d_adimissa);
        return {
          program,
          level: normalizeLevel(row.nivel_cursoal),
          status: normalizeText(row.status),
          admissionYear,
          endYear: extractYear(row.d_final) || extractYear(row.d_situacao),
        };
      })
      .filter((row) => row.program !== "ISO")
      .filter((row) => PROGRAM_ORDER.includes(row.program))
      .filter((row) => Boolean(row.level))
      .filter((row) => Boolean(row.admissionYear));
  }

  function isTerminalStatus(status) {
    return TERMINAL_STATUS_PREFIXES.some((prefix) => status.startsWith(prefix));
  }

  function buildActiveRows(baseRows, axisEndYear) {
    const result = [];
    baseRows.forEach((row) => {
      const startYear = Number(row.admissionYear);
      if (!Number.isFinite(startYear)) return;

      const endYearFromData = row.endYear ? Number(row.endYear) : Number.NaN;
      const hasEndYear = Number.isFinite(endYearFromData);
      let endYear = hasEndYear ? endYearFromData : axisEndYear;

      if (!hasEndYear && isTerminalStatus(row.status)) {
        // Fallback conservador para status terminal sem data de término.
        endYear = startYear;
      }
      ...
      const cappedEndYear = Math.min(endYear, axisEndYear);
  ...
  function createCard(level, total, cardIndex) {
    ...
    section.id = `card-ativos-${cardIndex}`;

    section.innerHTML = `
      <div class="stats-card-head">
        <h2>Alunos Ativos por Ano - ${level}</h2>
        <p>Registros aluno-ano ativos neste nível: ${total.toLocaleString("pt-BR")}.</p>
      </div>
      <div class="stats-card-grid">
        <div class="plot-panel plot-panel-large">
          <h3>Geral por Ano (Empilhado por Programa)</h3>
          <div id="ativos-geral-${cardIndex}" class="plot-chart plot-chart-large"></div>
        </div>
        <div class="plot-panel">
          <h3>Por Programa (7)</h3>
          <div id="ativos-mini-grid-${cardIndex}" class="mini-plots-grid"></div>
        </div>
      </div>
    `;
  ...
  function createAllProgramsCard(total) {
    const section = document.createElement("section");
    section.className = "stats-card";
    section.id = "card-ativos-geral-niveis";

    section.innerHTML = `
      <div class="stats-card-head">
        <h2>Alunos Ativos por Ano - Todos os Programas</h2>
        <p>Registros aluno-ano ativos (ISO excluído): ${total.toLocaleString("pt-BR")}.</p>
      </div>
      <div class="stats-card-grid">
        <div class="plot-panel plot-panel-large">
          <h3>Geral por Ano (Empilhado por Nível)</h3>
          <div id="ativos-geral-niveis-chart" class="plot-chart plot-chart-large"></div>
        </div>
      </div>
    `;
  ...
  function createAllLevelsByProgramCard(total) {
    const section = document.createElement("section");
    section.className = "stats-card";
    section.id = "card-ativos-geral-programas";

    section.innerHTML = `
      <div class="stats-card-head">
        <h2>Alunos Ativos por Ano - Mestrado + Doutorado (Por Programa)</h2>
        <p>Total de registros aluno-ano ativos considerados: ${total.toLocaleString("pt-BR")}.</p>
      </div>
      <div class="stats-card-grid">
        <div class="plot-panel plot-panel-large">
          <h3>Geral por Ano (Empilhado por Programa - Valores Absolutos)</h3>
          <div id="ativos-geral-programas-abs-chart" class="plot-chart plot-chart-large"></div>
        </div>
        <div class="plot-panel plot-panel-large">
          <h3>Geral por Ano (Empilhado por Programa - Proporcional 100%)</h3>
          <div id="ativos-geral-programas-prop-chart" class="plot-chart plot-chart-large"></div>
        </div>
      </div>
    `;
  ...
  function createRecentLevelCard(level, total, cardIndex, years) {
    const section = document.createElement("section");
    section.className = "stats-card";
    section.id = `card-ativos-${cardIndex}-ultimos-${LEVEL_CHART_LAST_YEARS}`;

    section.innerHTML = `
      <div class="stats-card-head">
        <h2>Alunos Ativos por Ano - ${level} (Últimos ${LEVEL_CHART_LAST_YEARS} Anos)</h2>
        <p>Registros aluno-ano ativos neste nível: ${total.toLocaleString("pt-BR")}. Eixo anual: ${years[0]} a ${years[years.length - 1]}.</p>
      </div>
      <div class="stats-card-grid">
        <div class="plot-panel plot-panel-large">
          <h3>Geral por Ano (Empilhado por Programa - Valores Absolutos)</h3>
          <div id="ativos-recente-${cardIndex}" class="plot-chart plot-chart-large"></div>
        </div>
        <div class="plot-panel plot-panel-large">
          <h3>Geral por Ano (Empilhado por Programa - Proporcional 100%)</h3>
          <div id="ativos-recente-prop-${cardIndex}" class="plot-chart plot-chart-large"></div>
        </div>
      </div>
    `;
  ...
  function renderLargeChart(targetId, rows, years) {
    ...
    const traces = series.map((item) => ({
      ...
      hovertemplate: `${item.program}<br>Ano: %{x}<br>Ativos: %{y}<extra></extra>`,
    }));
  ...
  function renderLargeChartProportional(targetId, rows, years) {
    ...
    const traces = series.map((item) => ({
      ...
      hovertemplate:
        ...
        `Ativos: %{customdata[0]} de %{customdata[1]}<extra></extra>`,
    }));
  ...
  function renderAllProgramsByLevelChart(targetId, rows, years) {
    ...
    const traces = series.map((item) => ({
      ...
      hovertemplate: `${item.level}<br>Ano: %{x}<br>Ativos: %{y}<extra></extra>`,
    }));
  ...
  function renderMiniCharts(gridId, rows, years, cardIndex) {
    ...
    PROGRAM_ORDER.forEach((program) => {
      const chartId = `ativos-mini-${cardIndex}-${program.toLowerCase()}`;
      ...
      const trace = {
        ...
        hovertemplate: `${program}<br>Ano: %{x}<br>Ativos: %{y}<extra></extra>`,
      };
  ...
  function dataYearBounds(rows) {
    const numericYears = rows
      .flatMap((row) => {
        ...
        const endYear = row.endYear ? Number(row.endYear) : Number.NaN;
        if (Number.isFinite(admissionYear)) years.push(admissionYear);
  ...
  function createActiveStudentsTableCard(rows, axisEndYear) {
    ...
    const section = document.createElement("section");
    section.className = "stats-card";
    section.id = "card-ativos-tabela";
    section.innerHTML = `
      <div class="stats-card-head">
        <h2>Alunos Ativos por Programa (Últimos ${MATRIX_LAST_YEARS} Anos)</h2>
      ...
      <div class="admissao-matrix-wrap">
        <table class="admissao-matrix" aria-label="Tabela de alunos ativos por programa e ano">
          <thead>
            <tr>
  ...
  async function init() {
    ...
    setStatus("Processando dados de alunos ativos por ano...");
    const rawRows = await loadRows();
    const baseRows = prepareRows(rawRows);
    const bounds = dataYearBounds(baseRows);
    if (!bounds) {
      throw new Error("Nenhum ano de admissão válido encontrado para cálculo de ativos.");
    }
    ...
    const axisEndYear = Number.isFinite(mdbCurrentYear)
      ...
      : bounds.max;
    const rows = buildActiveRows(baseRows, axisEndYear);
    if (rows.length === 0) {
      throw new Error("Nenhum registro ativo foi gerado com os critérios informados.");
    }
    ...
    renderAllProgramsByLevelChart("ativos-geral-niveis-chart", rows, fullYearAxis);

    const allLevelsByProgramCard = createAllLevelsByProgramCard(rows.length);
    cardsRoot.appendChild(allLevelsByProgramCard);
    renderLargeChart("ativos-geral-programas-abs-chart", rows, fullYearAxis);
    renderLargeChartProportional("ativos-geral-programas-prop-chart", rows, fullYearAxis);

    const tableCard = createActiveStudentsTableCard(rows, axisEndYear);
    ...
    levels.forEach((level, index) => {
      ...
      renderLargeChart(`ativos-recente-${index}`, levelRows, levelRecentAxis);
      renderLargeChartProportional(`ativos-recente-prop-${index}`, levelRows, levelRecentAxis);
    ...
    levels.forEach((level, index) => {
      ...
      renderLargeChart(`ativos-geral-${index}`, levelRows, fullYearAxis);
      renderMiniCharts(`ativos-mini-grid-${index}`, levelRows, fullYearAxis, index);
      cardsRendered += 1;
    });

    setStatus(
      `Exibindo ${rows.length.toLocaleString("pt-BR")} registros aluno-ano ativos ` +
        `(a partir de ${baseRows.length.toLocaleString("pt-BR")} alunos com admissão válida e ISO excluído), ` +
        `${cardsRendered} painel(is) por nível, ` +
  ...
  init().catch((error) => {
    console.error(error);
    setStatus(`Erro ao gerar painel de alunos ativos: ${error.message}`, true);
  });
})();
