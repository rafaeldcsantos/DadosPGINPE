(function () {
  const statusEl = document.getElementById("anais-status");
  const summaryEl = document.getElementById("anais-resumo");
  const chartComAmbosEl = document.getElementById("anais-chart");
  const miniGridEl = document.getElementById("anais-mini-grid");
  ...
  async function loadCsvText() {
    ...
    if (!response.ok) {
      throw new Error(
        `Falha ao carregar Data/inpe_anais_autores_por_artigo_2013_2024_glosa0_simplificado_flags.csv (HTTP ${response.status}).`
      );
    }
  ...
  function renderOverallChart(yearAxis, aggregated) {
    ...
    window.Plotly.newPlot(
      ...
      traces,
      layout,
      plotConfig("dadospginpe_anais_com_egressos_ou_discentes")
    );
  }
  ...
  function renderProgramCharts(yearAxis, rows, programCatalog) {
    ...
    programCatalog.forEach((program) => {
      const chartId = `anais-mini-${program.sigla.toLowerCase()}`;
      ...
      window.Plotly.newPlot(
        ...
        traces,
        layout,
        plotConfig(`dadospginpe_anais_${program.sigla.toLowerCase()}`)
      );
    });
  }
  ...
  async function init() {
    ...
    setStatus("Carregando e agregando artigos em anais por ano...");
    ...
    setStatus(
      `Exibindo ${total.toLocaleString("pt-BR")} artigos em anais entre ${yearAxis[0]} e ${yearAxis[yearAxis.length - 1]}. ` +
  ...
  init().catch((error) => {
    console.error(error);
    setStatus(`Erro ao carregar produção em anais: ${error.message}`, true);
